...
def should_stop_early(relevant_docs: List[Any], confidence_scores: List[float]) -> bool:
    ...
    high_confidence_count = sum(1 for score in confidence_scores 
                               if score >= GRADING_CONFIG["confidence_threshold"])
...
def grade_documents(state: GraphState) -> Dict[str, Any]:
    ...
    try:
        ...
        graded_results = grade_document_batch(
            documents, 
            question, 
            batch_size=GRADING_CONFIG["batch_size"]
        )
        ...
        logger.info(f"Results: {len(filtered_docs)}/{len(documents)} documents passed "
