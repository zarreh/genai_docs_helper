...
def parallel_retrieve(queries: List[str], timeout_per_query: int = 5) -> List[Document]:
    ...
    with ThreadPoolExecutor(max_workers=min(5, len(queries))) as executor:
        ...
        future_to_query = {
            executor.submit(standard_retriever.invoke, query): query 
    ...
    elapsed_time = time.time() - start_time
    logger.info(f"Parallel retrieval completed: {successful_queries}/{len(queries)} queries succeeded, "
...
def execute_fast_retrieval_path(question: str, original_question: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if len(initial_docs) >= 10:  # Sufficient documents for fast path
            ...
            logger.info(f"Fast path successful: {len(reranked_docs)} documents in {result['performance_metrics']['retrieval_time']:.2f}s")
            return result
...
def execute_comprehensive_retrieval_path(question: str, original_question: str) -> Dict[str, Any]:
    ...
    try:
        ...
        reranked_documents = fast_semantic_rerank(
            original_question, 
            all_documents, 
            top_k=min(30, len(all_documents))
        )
        ...
        logger.info(f"Comprehensive retrieval completed: {len(final_documents)} final documents "
